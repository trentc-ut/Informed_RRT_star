...
void PublishPredictedScan() {
  ...
  const uint32_t kColor = 0x10cce0;
...
void DrawObservations(const vector<float>& ranges, float angle_min, float angle_max) {
	const uint32_t kColor = 0x10cce0;
    Vector2f robot_loc(0, 0);
    float robot_angle(0);
    particle_filter_.GetLocation(&robot_loc, &robot_angle);
    robot_loc += Eigen::Rotation2Df(robot_angle) * Vector2f(0.2, 0.0);
	float ang_stepSize = (angle_max-angle_min) / (ranges.size()-1);
	for (size_t k=0 ; k<ranges.size() ; k+=20) {
		if (ranges[k]<30.0) {
			float totBng = robot_angle + angle_min + ang_stepSize*k;
			DrawLine(robot_loc, robot_loc+Vector2f(ranges[k]*cosf(totBng),ranges[k]*sinf(totBng)), kColor, vis_msg_);
		}
	}
}
...
void LaserCallback(const sensor_msgs::LaserScan& msg) {
  ...
  last_laser_msg_ = msg;
  particle_filter_.ObserveLaser(msg.ranges,
								msg.range_min,
								msg.range_max,
								msg.angle_min,
								msg.angle_max);
  DrawObservations(msg.ranges, msg.angle_min, msg.angle_max);
